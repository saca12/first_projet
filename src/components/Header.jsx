...
export default function Header({
  ...
  return (
    <nav className="fixed top-0  backdrop-blur-sm nav-bar flex justify-between items-center w-[100vw] h-16">
      <div className="flex items-center justify-between w-[90%] h-full border-b border-gray-600 mx-auto">
        {/* SECTION GAUCHE : LOGO + NAV */}
        <section className="flex items-center gap-2">
          <Logo />
          <ul className="flex gap-6 text-sm font-medium border-l-2 border-[#7BA0FF] pl-5">
            {links.map((link) => (
              <li
                key={link.name}
                onClick={() => onLinkClicked(link.name, link.path)}
                className={`cursor-pointer transition-colors ${
                  activePage === link.name
                    ? "text-[#9EB1FF] font-semibold"
                    : "text-[#E2DDFE] hover:text-[#9EB1FF]"
                }`}
              >
                {link.name}
              </li>
        ...
        {/* SECTION DROITE : THEME TOGGLE + USER */}
